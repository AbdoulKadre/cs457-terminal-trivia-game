...
**Student Name:** Abdoul Kadre Tiemtore  
**Date:** 2027-09-20  
**Course:** CS 457 - Computer Networks  
**Target Server Domain:** `server.tiemtore.edu`  
...
### 1.1 Game Overview
- **Chosen Game:** Terminal Trivia (Question-and-answer quiz with round scoring and leaderboards)
- **Player Capacity:** 2 Players (Simulated via 2 CML Client nodes)
- **Game Summary:** Terminal Trivia is a two-player, round-based quiz game played in the console. A central server manages the game, including questions, turn order, scoring, and leaderboard updates after each round. Players connect as clients, answering questions displayed by the server. Each match has 10 rounds, with the same multiple-choice question (A–D) sent to both players. The server collects answers in order, awards points for correct responses, and updates the leaderboard. After 10 rounds, the player with the highest score wins. Questions are randomly selected from a predefined bank, ensuring no repeats during a match.

### 1.2 Core Game Rules & Win/Draw Conditions
- **Turn Mechanics:** The server assigns Player 1 to the first client and Player 2 to the second. Each player can choose a display name (or defaults to "Player_1" or "Player_2"). A third connection is rejected. The game starts only when two players are connected. Players take turns answering the same question, with Player 1 starting first in odd rounds and Player 2 in even rounds. Only the active player can submit an answer; the other must wait. If a player tries to answer out of turn, it’s rejected. Each player has 20 seconds to answer. If time runs out, the answer is recorded as a non-answer, and the turn moves on. Correct answers earn 10 points, while incorrect or timed-out answers earn 0. The maximum score is 100 points.
- **Victory Condition:** After 10 rounds, the player with the highest score wins. The server determines the result and broadcasts GAME_OVER containing the winner's name, both final scores, correct-answer counts, and the final leaderboard.
- **Draw/Tie Condition:** If cumulative scores are equal after round 10, the match enters sudden-death tiebreak of up to five additional rounds, played under the same rules and continuing the turn alternation (Player 1 first in tiebreak round 1, Player 2 in round 2, Player 1 in round 3). If all three sudden-death rounds are inconclusive, the match is declared an official DRAW and the server broadcasts GAME_OVER with a result of DRAW and the tied final scores. Disconnection. If a player disconnects mid-match, the server notifies the remaining player and ends the match as incomplete
...
- **Router R2 Authoritative DNS:** Configured with `ip dns server` and static host mapping `server.tiemtore.edu` -> `192.168.20.100`.
...
- **Cisco Infrastructure Configuration:** Router R1 DHCP pools (`CLIENT1_POOL`, `CLIENT2_POOL`) and Router R2 authoritative DNS (`ip host server.tiemtore.edu 192.168.20.100`).
